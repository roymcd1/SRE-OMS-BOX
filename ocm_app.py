...
Supports:
  ...
  - Specific date filter with ?date=YYYYMMDD
"""
...
def get_schedule():
    ...
    Optional query parameters:
      ?todayOnly=true   → returns only today's schedule
      ?date=YYYYMMDD    → returns schedule for a specific date
    """
    ...
    if today_only:
        ...
        print(f"📅 Filtering to date: {specific_date}")
        primary_data = [p for p in primary_data if p["Date"] == specific_date]
        standby_data = [s for s in standby_data if s["Date"] == specific_date]
